...
class AnswerController extends AdminControllerLogic
{
     ...
     * @param $id
     */
    public function adminRecovery($user, $ids, $id)
    {
        if(is_numeric($id) && empty($id)){
            $list = (new $this->model)->onlyTrashed()->restore();
            //还原评论
            AskReply::where('theme_id', $list->pluck('id'))->where('type', 1)->onlyTrashed()->restore();
        }else{
            (new $this->model)->whereIn($this->key,$ids)->onlyTrashed()->restore();
            //还原评论
            AskReply::whereIn('theme_id', $ids)->where('type', 1)->onlyTrashed()->restore();
        }
    }
    ...
    public function adminRecoveryDelete($user, $ids, $id)
    {
        ...
        foreach ($list as $item) {
            AskCommentLogic::deleteAnswer($item->id);
        }
    }
